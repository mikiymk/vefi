...
pub fn AvlTreeNode(T: type) type {
    return struct {
        item: T,
       ...
       h = @max(h,       left.height() );
                ...
                const right = if (node.right) |right|   {
...
/// ```
            fn rotateRight(node: *Node) !*Node {
                const a = node;
                const b = a.left orelse return error.NodeNotFound;
                const c = b.right;
